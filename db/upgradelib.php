...
/**
 * This method finds all courses in 'event' format that have actual number of sections
 * bigger than their 'numsections' course format option.
 * For each such course we call {@link format_event_upgrade_hide_extra_sections()} and
 * either delete or hide "orphaned" sections.
 */
function format_event_upgrade_remove_numsections() {
    global $DB;
    ...
    $params = ['format1' => 'event', 'format2' => 'event', 'numsections' => 'numsections'];
    ...
    foreach ($needfixing as $courseid => $numsections) {
        format_event_upgrade_hide_extra_sections($courseid, $numsections);
    }

    $DB->delete_records('course_format_options', ['format' => 'event', 'sectionid' => 0, 'name' => 'numsections']);
}
 ...
 */
function format_event_upgrade_hide_extra_sections($courseid, $numsections) {
    global $DB;
