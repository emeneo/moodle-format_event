...
/**
 * format_event related unit tests
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_event_testcase extends advanced_testcase {

    /**
     * Tests for format_event::get_section_name method with default section names.
     */
    public function test_get_section_name() {
        ...
        $numsections = 5;
        $course = $generator->create_course(array('numsections' => $numsections, 'format' => 'event'),
    ...
    /**
     * Tests for format_event::get_section_name method with modified section names.
     */
    ...
    /**
     * Tests for format_event::get_default_section_name.
     */
    public function test_get_default_section_name() {
        ...
        foreach ($coursesections as $section) {
            if ($section->section == 0) {
                $sectionname = get_string('section0name', 'format_event');
                $this->assertEquals($sectionname, $courseformat->get_default_section_name($section));
            } else {
                $sectionname = get_string('sectionname', 'format_event') . ' ' . $section->section;
                $this->assertEquals($sectionname, $courseformat->get_default_section_name($section));
            }
        }
    }
    ...
    public function test_update_inplace_editable() {
        ...
        $this->setUser($user);
        $course = $this->getDataGenerator()->create_course(array('numsections' => 5, 'format' => 'event'),
            array('createsections' => true));
        $section = $DB->get_record('course_sections', array('course' => $course->id, 'section' => 2));

        // Call webservice without necessary permissions.
        try {
            core_external::update_inplace_editable('format_event', 'sectionname', $section->id, 'New section name');
        ...
        $res = core_external::update_inplace_editable('format_event', 'sectionname', $section->id, 'New section name');
    ...
    public function test_inplace_editable() {
        ...
        // Call callback format_event_inplace_editable() directly.
        $tmpl = component_callback('format_event', 'inplace_editable', array('sectionname', $section->id, 'Rename me again'));
    ...
    public function test_default_course_enddate() {
        ...
        $params = array('format' => 'event', 'numsections' => 5, 'startdate' => 1445644800);
    ...
    public function test_get_view_url() {
        ...
        $generator = $this->getDataGenerator();
        $course1 = $generator->create_course(array('format' => 'event'));
